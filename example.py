...
for line in f:
    noaa_verified.append(float(line.split()[3]))
    noaa_predicted.append(float(line.split()[2]))
f.close()

print(f"my predicted = ({my_prediction.size}) {my_prediction}\n")
print(f"verified = ({len(noaa_verified)}) {noaa_verified}\n")
print(f"noaa_predicted = ({len(noaa_predicted)}) {noaa_predicted}\n")
